...
void keySettingsSetup(){keySettingsEnsure("nano:button","M5NanoC6 Button","/chainoscnano/nano/button");Preferences p;String known;if(p.begin("keycfg",true)){known=p.getString("known","");p.end();}loadingKnown=true;int o=0;while(o<(int)known.length()){int e=known.indexOf('\n',o);if(e<0)e=known.length();String id=known.substring(o,e);if(id.startsWith("chain:")&&id.length()>6){String uid=id.substring(6);keySettingsEnsure(id,String("Chain Key ")+uid,String("/chainoscnano/chain/key/")+uid);}o=e+1;}loadingKnown=false;}
