...
bool loadSetting(KeySetting&s){const DeviceFileLoadResult result=deviceFileStorageLoad(s);if(result==DeviceFileLoadResult::Loaded)return true;if(result==DeviceFileLoadResult::Error)return false;if(compactStorageLoad(compactStorageNamespace(s.identity),s)){if(deviceFileStorageSave(s))NANO_STORAGE_LOGF("[ChainOSCnano][KEYCFG] migrated identity=%s source=NVS target=LittleFS\n",s.identity.c_str());return true;}return false;}
bool writeSetting(const KeySetting&s){if(!deviceFileStorageSave(s))return false;KeySetting v=s;if(deviceFileStorageLoad(v)!=DeviceFileLoadResult::Loaded){NANO_STORAGE_LOGF("[ChainOSCnano][LITTLEFS] key_verify uid=%s result=decode_failed\n",s.identity.c_str());return false;}return sameSetting(s,v);}
void saveKnown(){/* LittleFS files are the catalog; NVS is migration-only. */}
}

void keySettingsNormalizeSequence(KeySequenceConfig&s){if(!isfinite(s.start))s.start=0;if(!isfinite(s.end))s.end=10;if(!isfinite(s.step)||fabsf(s.step)<1e-9f)s.step=1;if(s.start<=s.end&&s.step<0)s.step=-s.step;if(s.start>s.end&&s.step>0)s.step=-s.step;s.current=s.start;s.direction=SequenceDirection::Forward;}
void keySettingsSetup(){deviceFileStorageBegin();keySettingsEnsure("nano:button","M5NanoC6 Button","/chainoscnano/nano/button");String fileIdentities[MAX_KEY_SETTINGS];size_t fileCount=deviceFileStorageList("key",fileIdentities,MAX_KEY_SETTINGS);loadingKnown=true;for(size_t i=0;i<fileCount;++i){String id=fileIdentities[i];if(id.startsWith("chain:")&&id.length()>6){String uid=id.substring(6);keySettingsEnsure(id,String("Chain Key ")+uid,String("/chainoscnano/chain/key/")+uid);}}loadingKnown=false;Preferences p;String known;if(p.begin("keycfg",true)){known=p.getString("known","");p.end();}loadingKnown=true;int o=0;while(o<(int)known.length()){int e=known.indexOf('\n',o);if(e<0)e=known.length();String id=known.substring(o,e);if(id.startsWith("chain:")&&id.length()>6){String uid=id.substring(6);keySettingsEnsure(id,String("Chain Key ")+uid,String("/chainoscnano/chain/key/")+uid);}o=e+1;}loadingKnown=false;}
KeySetting* keySettingsEnsure(const String&id,const String&name,const String&address){for(size_t i=0;i<settingCount;++i)if(settings[i].identity==id)return &settings[i];if(settingCount>=MAX_KEY_SETTINGS)return nullptr;KeySetting&s=settings[settingCount++];s.identity=id;s.displayName=name;s.builtIn=id=="nano:button";s.pressMessages[0].address=address;s.pressMessages[0].valueType=TYPE_INT;s.pressMessages[0].valueStr="1";s.releaseMessages[0].address=address;s.releaseMessages[0].valueType=TYPE_INT;s.releaseMessages[0].valueStr="0";s.sequence.address=address;keySettingsNormalizeSequence(s.sequence);loadSetting(s);if(!s.builtIn)saveKnown();return &s;}
...
bool keySettingsDelete(const String&id){size_t found=settingCount;for(size_t i=0;i<settingCount;++i)if(settings[i].identity==id){if(settings[i].builtIn||settings[i].connectedPortMask)return false;found=i;break;}if(found==settingCount||!deviceFileStorageRemove("key",id))return false;compactStorageDelete(id);for(size_t i=found+1;i<settingCount;++i)settings[i-1]=settings[i];--settingCount;settings[settingCount]=KeySetting();saveKnown();return true;}
