...
bool load(const String& identity, JoystickSetting& setting, bool& found) {
  found=false; if(compactStorageLoad(compactStorageNamespace(identity),setting)){found=true;return true;}
  return false;

}
bool write(const JoystickSetting& s){const String ns=compactStorageNamespace(s.identity);if(!compactStorageSave(ns,s))return false;JoystickSetting v=s;bool found=false;return load(s.identity,v,found)&&found&&same(s,v);}
void saveKnown(){if(loadingKnown)return;String known;for(size_t i=0;i<settingCount;++i){if(!known.isEmpty())known+='\n';known+=settings[i].identity;}Preferences p;if(p.begin("joycfg",false)){p.putString("known",known);p.end();}}
}
void joystickSettingsSetup(){Preferences p;String known;if(p.begin("joycfg",true)){known=p.getString("known","");p.end();}loadingKnown=true;int offset=0;while(offset<(int)known.length()){int end=known.indexOf('\n',offset);if(end<0)end=known.length();String id=known.substring(offset,end);if(id.startsWith("chain:")&&id.length()>6)joystickSettingsEnsure(id,String("Chain Joystick ")+id.substring(6));offset=end+1;}loadingKnown=false;NANO_VERBOSE_LOGF("[ChainOSCnano][JOYCFG] setup_complete settings=%u\n",(unsigned)settingCount);}
...
bool joystickSettingsDelete(const String& identity){size_t found=settingCount;for(size_t i=0;i<settingCount;++i)if(settings[i].identity==identity){if(settings[i].connectedPortMask)return false;found=i;break;}if(found==settingCount)return false;compactStorageDelete(identity);for(size_t i=found+1;i<settingCount;++i)settings[i-1]=settings[i];--settingCount;settings[settingCount]=JoystickSetting();saveKnown();return true;}
